...
contract('AddTokenToBehodler', async function (accounts) {
    ...
    it('adding burnable and non burnable tokens to behodler succeeds', async function () {
        ...
        //bond user to minion
        await this.powersRegistry.bondUserToMinion(adder, stringToBytes("Glaurung"), { from: Melkor })
        //pour power to adder
        await this.powersRegistry.create(stringToBytes('ADD_TOKEN_TO_BEHODLER'), stringToBytes('LACHESIS'), true, false, { from: Melkor })
        await this.powersRegistry.pour(stringToBytes('ADD_TOKEN_TO_BEHODLER'), stringToBytes('Glaurung'), { from: Melkor })
        ...
        await this.angband.mapDomain(this.lachesis.address, stringToBytes('LACHESIS'), { from: Melkor })
        this.invoker = await Invoker.new(this.burnableToken.address, true, stringToBytes('ADD_TOKEN_TO_BEHODLER'), this.angband.address, { from: adder })
        ...
        await this.angband.authorizeInvoker(this.invoker.address, true, { from: Melkor })

        //assert it isn't on behodler
        const burnableBefore = await this.behodler.tokenBurnable.call(this.burnableToken.address)
        const validBefore = await this.behodler.validTokens.call(this.burnableToken.address)
        ...
        const burnableAfter = await this.behodler.tokenBurnable.call(this.burnableToken.address)
        const validAfter = await this.behodler.validTokens.call(this.burnableToken.address)
